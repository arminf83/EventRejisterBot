...
لطفاً اطلاعات پروفایل خودت رو تکمیل کن:
...
async def menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🎯 مشاهده رویدادها":
        events = await sync_to_async(list)(Event.objects.filter(active=True).order_by("main_date"))
        if not events:
            await update.message.reply_text("رویدادی فعال نیست.")
            return MENU

        for ev in events:
            main_local = timezone.localtime(ev.main_date) if ev.main_date else None
            caption_lines = [f"📌 {ev.title}"]

            if main_local:
                j_main = jdatetime.datetime.fromgregorian(datetime=main_local.replace(tzinfo=None))
                caption_lines.append(f"📅 تاریخ: {j_main.strftime('%Y/%m/%d')} | {j_main.strftime('%H:%M')}")
...
async def reminder_job(app):
    while True:
        try:
            ...
            events = await sync_to_async(list)(
                Event.objects.prefetch_related("registrations__participant").filter(
                    main_date__gte=now,
                    main_date__lte=now + timedelta(hours=24),
                    active=True
                )
            )
        ...
        await asyncio.sleep(60)  # ✅ اصلاح شد: هر ۱ ساعت
