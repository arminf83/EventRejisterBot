...
خوشحالیم که شما را در کنارمون داریم. 🙏
برای شروع لطفاً اطلاعات پروفایل خودت رو تکمیل کن 
نام و نام خانوادگی کامل رو وارد کن:
"""
            )
...
async def register_major(update, context):
    ...
    user.major = text
    await sync_to_async(user.save)()
    await update.message.reply_text("نحوه آشناییتون با مجموعه gita چگونه بوده ")
            ...
            for att in attachments:
                try:
                    with open(att.file.path, "rb") as f:
                        await context.bot.send_document(chat_id=chat_id, document=f, caption=att.description or "")
                except Exception as e:
                    print("File send error:", e)

            await asyncio.sleep(0.5)  # جلوگیری از تایم‌اوت یا بلاک شدن
...
async def reminder_job(app):
    while True:
        ...
        await asyncio.sleep(30)  # هر ۱ ساعت بررسی


# ------------------------------------------
#  پاسخ به حضور / عدم حضور
# ------------------------------------------
